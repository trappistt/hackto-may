...
import { coachChatDescription, coachChatTitle, coachStarters } from "@/components/onboarding/toneCopy.js";
...
export default function CoachChat({
  ...
  const starters = coachStarters(tone);
    ...
    <>
      ...
      <ScrollArea
        className={cn(
          ...
          compact ? "h-[280px]" : embedded ? "h-[min(360px,45vh)]" : "h-[min(420px,50vh)]"
        )}
      >
  ...
  return (
    ...
    >
      <CardHeader className={cn("pb-3", compact && "p-4 pb-2")}>
        ...
        <CardTitle className="font-display text-xl">{coachChatTitle(tone)}</CardTitle>
        <CardDescription>{coachChatDescription(tone)}</CardDescription>
      </CardHeader>

      <CardContent className="flex min-h-0 flex-1 flex-col gap-4 pb-4">{body}</CardContent>
    </Card>
  );
}
